...
import           Combinators (Parser (..), Result (..), fail',
                              satisfy, symbol, runParser, stream, (<?>))
...
parseExpr = "Expected expression" <?> uberExpr operators (parseCall <|> Num <$> parseNum <|> Ident <$> parseIdent <|> symbol '(' *> parseExpr <* symbol ')') BinOp UnaryOp

-- Парсер для целых чисел
parseNum :: Parser String String Int
parseNum = "Expected number" <?> foldl (\acc d -> 10 * acc + digitToInt d) 0 <$> go
  where
    go :: Parser String String String
    go = some (satisfy isDigit)

parseIdent :: Parser String String String
parseIdent = "Expected identifier" <?> ((:) <$> (satisfy isLetter <|> symbol '_')) <*> many (satisfy isLetter <|> satisfy isDigit <|> symbol '_')


parseArgss:: Parser String String [AST]
parseArgss = ((:) <$> parseExpr <*> many (parseStr ", " *> parseExpr)) <|> (pure [])
